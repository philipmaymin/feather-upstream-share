...
test('mirrors parent and child execution across completion, replay, and responsive layouts', async ({ page }) => {
  ...
  await expect(chatPanel.getByTestId('omp-parent-execution-summary')).toContainText('Testing current execution segment')
  await expect(parentExecution.getByText('Child-only reasoning stays in the inspector.')).toHaveCount(0)
  await expect(parentExecution.getByTestId('omp-tool-card')).toHaveCount(0)
  await expect(chatPanel.getByTestId('working-indicator')).toBeHidden()
  await page.screenshot({ path: '/tmp/feather-compact-work-mobile.png', fullPage: false })

  const agentsTab = page.getByRole('button', { name: /Agents 1/ })
  await expect(agentsTab).toBeVisible()
  await parentExecution.click()
  const agentsPanel = page.getByTestId('agents-panel')
  await expect(agentsPanel).toBeVisible()
  const detailedParentExecution = agentsPanel.getByTestId('omp-parent-execution')
  await expect(detailedParentExecution.getByTestId('omp-tool-card')).toHaveCount(3)
  await expect(detailedParentExecution).toContainText('Testing current execution segment')
  const currentTool = detailedParentExecution.getByTestId('omp-tool-card').filter({ hasText: 'Testing current execution segment' })
  ...
  await expect(page.getByTestId('todos-panel')).toContainText('Verify child inspector')
  await page.getByRole('button', { name: 'Chat', exact: true }).click()
  await expect(chatPanel.getByTestId('omp-parent-execution')).toContainText('Reading parent bridge state')
