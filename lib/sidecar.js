...
import { tmuxKeyMatchesSession } from './tmux-session.js';
...
export function roomGroupId(roomName) {
...
// Room groups are durable: resident state owns membership while the existing
// append-only thread and sequence survive every roster synchronization.
export function syncRoomGroup({ roomName, members }) {
  const id = roomGroupId(roomName);
  const roles = [];
  for (const member of members) { assertRole(member.role, roles); roles.push(member.role); }
  ensureDir();
  fs.mkdirSync(groupDir(id), { recursive: true });
  let result;
  GROUPS_STATE.update((groups) => {
    ...
    result = {
      ...
      kind: 'room',
      roomName,
      members: members.map((member) => ({ ...member, spawned: false })),
      agent: 'mixed',
      task: `Permanent A2A thread for #${roomName}`,
      status: 'active',
      seq: existing?.seq || 0,
...
export function renameRoomGroup(oldRoomName, newRoomName) {
  const oldId = roomGroupId(oldRoomName);
  const newId = roomGroupId(newRoomName);
  const groups = GROUPS_STATE.read();
  const existing = groups[oldId];
  if (!existing) return null;
  if (groups[newId]) throw new Error(`sidecar group already exists: ${newId}`);
  const oldDir = groupDir(oldId);
  const newDir = groupDir(newId);
  if (fs.existsSync(newDir)) throw new Error(`sidecar directory already exists: ${newId}`);
  fs.renameSync(oldDir, newDir);
  let result;
  try {
    GROUPS_STATE.update((current) => {
      if (!current[oldId] || current[newId]) throw new Error('Room sidecar changed during rename');
      const next = { ...current };
      delete next[oldId];
      result = {
        ...existing,
        ...
        task: `Permanent A2A thread for #${newRoomName}`,
      };
      next[newId] = result;
      return next;
    });
...
export function markMembersPrimed(id, memberKeys) {
  GROUPS_STATE.update((groups) => {
    const group = groups[id];
    if (!group) return groups;
    const primedMembers = [...new Set([...(group.primedMembers || []), ...memberKeys])];
    return { ...groups, [id]: { ...group, primedMembers } };
  });
}
...
export function resolveRecipients(group, to, senderRole) {
  if (typeof to !== 'string') return { targets: [], missing: ['invalid address'] };
  if (to === ALL) {
    return { targets: group.members.filter(m => m.role !== senderRole), missing: [] };
  }
  const roles = [...new Set(to.split(',').map(s => s.trim()).filter(Boolean))];
  const targets = [];
  const missing = [];
  for (const role of roles) {
    if (role === senderRole) continue; // never self-inject (livelock guard, mirrors `all`)
    const member = group.members.find(candidate => candidate.role === role);
    if (member) targets.push(member); else missing.push(role);
  }
...
export function groupForSessionPrefix(prefix) {
  return listGroups().find(g =>
    g.status === 'active' && g.members.some(m => tmuxKeyMatchesSession(prefix, m.sessionId))) || null;
}
...
export function groupForSenderAndRole(prefix, to) {
  const candidates = listGroups().filter(g =>
    g.status === 'active' && g.members.some(m => tmuxKeyMatchesSession(prefix, m.sessionId)));
...
export function roleForPrefix(group, prefix) {
  const m = group.members.find(m => tmuxKeyMatchesSession(prefix, m.sessionId));
...
export function appendMessage(id, msg) {
  if (!msg || typeof msg.from !== 'string' || typeof msg.to !== 'string' || typeof msg.text !== 'string') {
    throw new Error('invalid sidecar message');
  }
  ensureDir();
  fs.mkdirSync(groupDir(id), { recursive: true });
  let seq;
...
export function formatInbound(groupId, message) {
  return `[feather-sidecar ${groupId} ${message.seq} ${message.from}] ${JSON.stringify(message.text)}`;
}
