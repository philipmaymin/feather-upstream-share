...
// Room status reporters are implementation workers, not ordinary conversations.
// Mark new workers explicitly while retaining title checks for older releases.
export function sessionIsRoomPulse(meta) {
  return meta?.background === 'room-pulse'
    || /^(Keep working|Status): #[a-z0-9][a-z0-9-]*$/.test(String(meta?.title || ''));
}
