...
const SIDECAR_ENVELOPE_RE = /^\[feather-sidecar [^\]]+\](?:\s|$)/

function isInternalSidecarText(text) {
  return typeof text === 'string' && SIDECAR_ENVELOPE_RE.test(text.trim())
}
...
export function parseMessage(line) {
  try {
    ...
    if (blocks.length === 1 && blocks[0]?.type === 'text' && isInternalSidecarText(blocks[0].text)) return null
...
export function parseCodexMessage(line) {
  try {
    ...
    if (p.type === 'message') {
      ...
      if (!text) return null
      if (isInternalSidecarText(text)) return null
...
function normalizeOmpTodoDetails(details) {
  if (!details || typeof details !== 'object' || !Array.isArray(details.phases)) return null
  const phases = details.phases.slice(0, 30).map(phase => ({
  ...
  return {
    phases,
    ...(typeof details.op === 'string' ? { op: details.op.slice(0, 20) } : {}),
  }
}
...
function buildToolResultBlocks(msg) {
  ...
  const todoDetails = msg.toolName === 'todo' ? normalizeOmpTodoDetails(msg.details) : null
  return [{
    type: 'tool_result',
...
export function parseOmpMessage(line) {
  try {
    ...
    return {
      ...
      timestamp: d.timestamp || new Date(msg.timestamp).toISOString(),
      content: blocks,
      ...(typeof msg.stopReason === 'string' && msg.stopReason ? { stopReason: msg.stopReason } : {}),
    }
  } catch { return null }
}
