...
function readExisting(file, config) {
  try {
    ...
    return { exists: true, bytes, value: parseBytes(bytes, config, file), mode: fs.statSync(file).mode & 0o777 }
  } catch (error) {
...
export function createJsonState({
  ...
  const config = { file, root, document, defaultValue, validate, mode, defaultMode, faultInjector }
  ...
  function commit(value, current, target, backup) {
    ...
    if (current.exists && current.bytes === bytes && current.mode === targetMode) return value
    if (current.exists) {
  ...
  function update(mutator) {
    return mutate((target, backup) => {
      const existing = readExisting(target, config)
      if (!existing.exists && fs.existsSync(backup)) {
        throw new JsonStateError(`missing ${document} at ${target} while a recovery copy exists; recover explicitly`)
      }
