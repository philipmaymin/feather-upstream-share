...
export class ChannelStore {
  ...
  addAgent({ channelId, actorId, username, displayName, sessionId, agentBackend = 'omp', makeDefault = false }) {
    ...
    const db = this.#requireWritable()
    db.prepare(`
      INSERT INTO principals(id, kind, username, display_name, avatar_seed, agent_backend, session_id, created_at)
      VALUES (?, 'agent', ?, ?, ?, ?, ?, ?)
      ON CONFLICT(username) DO UPDATE SET display_name = excluded.display_name,
        agent_backend = excluded.agent_backend, session_id = excluded.session_id
    `).run(principalId, clean, name, clean, agentBackend, session, nowIso())
    const principal = this.#principal(principalId) || mapPrincipal(db.prepare('SELECT * FROM principals WHERE username = ?').get(clean))
    const member = this.#addMembership({ channelId, actorId, principal, role: 'agent', notificationLevel: 'mentions' })
    if (makeDefault) db.prepare('UPDATE channels SET default_agent_id = ? WHERE id = ?').run(principal.id, channelId)
  ...
  #notifyThreadHumans(db, rootId, actorId, kind, reason, messageId = null) {
    ...
    const humans = db.prepare(`
      ...
      WHERE m.channel_id = ? AND m.left_seq IS NULL AND p.kind = 'human' AND p.id != ?
    `).all(thread.channel_id, actorId)
  ...
  updateThreadAttention({ rootId, principalId, action, value = true, until = null }) {
    const thread = this.getThread(rootId, principalId)
    const db = this.#requireWritable()
    db.prepare(`
    ...
    if (action === 'read') {
      ...
      db.prepare('UPDATE thread_reads SET following = ? WHERE principal_id = ? AND root_message_id = ?')
      ...
      db.prepare('UPDATE thread_reads SET snoozed_until = ? WHERE principal_id = ? AND root_message_id = ?')
        .run(until, principalId, rootId)
    } else {
      throw new Error('invalid thread attention action')
    }
    return this.getThread(rootId, principalId)
  }
  ...
  completeExecution({ executionId, content, timestamp }) {
    ...
    const needsYou = /^\\s*NEEDS YOU\\s*:/i.test(content)
    this.#transaction(db => {
      db.prepare(`
...
export { mentionedUsernames, threadTitle }
