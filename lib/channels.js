...
const PRESENTATION_FOCUSES = new Set(['intervention', 'delivery', 'review', 'direction'])
const PRESENTATION_FEEDBACK_ACTIONS = new Set([
  'helpful',
  'not_helpful',
  'prompt_prepared',
  'steering_prepared',
...
function deliverySummaries(rows) {
  const summaries = new Map()
  for (const row of rows) {
    if (!summaries.has(row.thread_root_id)) {
      summaries.set(row.thread_root_id, { queuedAgents: [], activeAgents: [], queuedCount: 0, activeCount: 0 })
    }
    const summary = summaries.get(row.thread_root_id)
    const active = row.state === 'processing'
    summary[active ? 'activeCount' : 'queuedCount'] += 1
    const agents = active ? summary.activeAgents : summary.queuedAgents
    if (!agents.some(agent => agent.id === row.agent_id)) {
      agents.push({ id: row.agent_id, username: row.agent_username, displayName: row.agent_display_name })
    }
  }
  return summaries
}
...
export class ChannelStore {
  ...
  agentSessionIds() {
    ...
    return new Set(this.db.prepare(`SELECT session_id FROM principals WHERE kind = 'agent' AND session_id IS NOT NULL`)
  ...
  #createNotifications(db, { channelId, rootId, messageId, author, content, messageType, seq }) {
    ...
    if (author.kind === 'human' && messageType === 'human') {
      ...
      if (!explicitAgents.length) {
        ...
        if (thread?.auto_agent_id) {
          const mainExecution = db.prepare(`
            SELECT thread_root_id FROM executions
            WHERE agent_principal_id = ? AND state IN ('queued', 'running')
            ORDER BY started_at LIMIT 1
          `).get(thread.auto_agent_id)
          const btw = mainExecution && mainExecution.thread_root_id !== rootId
            ? members.find(member => member.kind === 'agent' && member.display_name.toLowerCase() === 'btw')
            : null
          this.#enqueueDispatchDb(db, {
            channelId,
            rootId,
            messageId,
            agentId: btw?.id || thread.auto_agent_id,
            depth: 0,
          })
        }
      }
  ...
  #messageDepth(db, messageId) {
    ...
    `).get(rootId)?.count || 0)
    if (trigger?.author_kind === 'agent' && agentMessages >= MAX_AGENT_MESSAGES_PER_THREAD) return false
    ...
    `).run(id, `dispatch:${messageId}:${agentId}`, channelId, rootId, messageId, agentId,
      depth, queuedAt, queuedAt)
    const changed = Number(result.changes) > 0
    if (changed) db.prepare(`UPDATE threads SET state = 'working', updated_at = ? WHERE root_message_id = ?`)
      .run(queuedAt, rootId)
    return changed
  }
  ...
  listChannelRoots(channelId, principalId, limit = 100) {
    ...
    const rows = db.prepare(`
      SELECT m.*, p.kind AS author_kind, p.username AS author_username,
        ...
        EXISTS (
          ...
          WHERE n.recipient_id = ? AND n.thread_root_id = t.root_message_id
            AND n.kind = 'mention' AND n.read_at IS NULL AND n.done_at IS NULL
        ) AS mentioned
      FROM messages m JOIN principals p ON p.id = m.author_id
      JOIN payloads y ON y.id = m.payload_id
    ...
    `).all(principalId, principalId, channelId, Math.max(1, Math.min(200, Number(limit) || 100)))
    const deliveries = deliverySummaries(db.prepare(`
      SELECT o.thread_root_id, o.state, p.id AS agent_id, p.username AS agent_username,
        p.display_name AS agent_display_name
      FROM channel_outbox o JOIN principals p ON p.id = o.recipient_principal_id
      WHERE o.channel_id = ? AND o.state IN ('pending', 'processing')
      ORDER BY o.created_at
    `).all(channelId))
    const recoveries = new Map(db.prepare(`
      SELECT e.thread_root_id, e.id, e.state, e.error, p.id AS agent_id,
        p.username AS agent_username, p.display_name AS agent_display_name,
        EXISTS(SELECT 1 FROM channel_outbox o WHERE o.execution_id = e.id) AS can_restart
      FROM executions e JOIN principals p ON p.id = e.agent_principal_id
      WHERE e.channel_id = ? AND e.state IN ('error', 'killed')
        AND NOT EXISTS (
          SELECT 1 FROM executions newer
          WHERE newer.thread_root_id = e.thread_root_id AND newer.started_at > e.started_at
        )
    `).all(channelId).filter(row => row.can_restart).map(row => [row.thread_root_id, {
      id: row.id,
      state: row.state,
      error: row.error || null,
      agent: { id: row.agent_id, username: row.agent_username, displayName: row.agent_display_name },
      canRestart: true,
    }]))
    return rows.reverse().map(row => ({
      ...
      thread: {
        ...
        delivery: deliveries.get(row.id) || { queuedAgents: [], activeAgents: [], queuedCount: 0, activeCount: 0 },
  ...
  getThread(rootId, principalId) {
    ...
    `).all(rootId).map(row => ({
      ...
      error: row.error || null,
      canRestart: !!row.can_restart && ['running', 'error', 'killed'].includes(row.state),
    }))
    const delivery = deliverySummaries(db.prepare(`
      SELECT o.thread_root_id, o.state, p.id AS agent_id, p.username AS agent_username,
        p.display_name AS agent_display_name
      FROM channel_outbox o JOIN principals p ON p.id = o.recipient_principal_id
      WHERE o.thread_root_id = ? AND o.state IN ('pending', 'processing')
      ORDER BY o.created_at
    `).all(rootId)).get(rootId) || { queuedAgents: [], activeAgents: [], queuedCount: 0, activeCount: 0 }
    return {
      id: rootId,
      channelId: thread.channel_id,
      title: thread.title,
      state: thread.state,
      lastReadSeq: Number(read?.last_read_seq || 0),
  ...
  unreadActivityCount(principalId) {
    if (!this.db) return 0
    return Number(this.db.prepare(`
      SELECT COUNT(DISTINCT COALESCE(n.thread_root_id, n.id)) AS count
      FROM notifications n
      WHERE n.recipient_id = ? AND n.read_at IS NULL AND n.done_at IS NULL
        AND (n.thread_root_id IS NULL OR NOT EXISTS (
          SELECT 1 FROM thread_reads r WHERE r.principal_id = n.recipient_id
            AND r.root_message_id = n.thread_root_id
            AND r.snoozed_until IS NOT NULL AND r.snoozed_until > ?
        ))
    `).get(principalId, nowIso())?.count || 0)
  }
  ...
  activeExecutionSessionIds() {
    if (!this.db) return new Set()
    return new Set(this.db.prepare(`SELECT DISTINCT session_id FROM executions WHERE state = 'running'`)
      .all().map(row => row.session_id))
  }
  ...
  completeExecution({ executionId, content, timestamp }) {
    ...
    this.#transaction(db => {
      ...
      const moreWork = !!db.prepare(`
        SELECT 1 FROM channel_outbox
        WHERE thread_root_id = ? AND state IN ('pending', 'processing') LIMIT 1
      `).get(execution.thread_root_id)
      db.prepare(`
        UPDATE threads SET state = ?, updated_at = ? WHERE root_message_id = ?
      `).run(needsYou ? 'needs_you' : moreWork ? 'working' : 'resolved', nowIso(), execution.thread_root_id)
      if (needsYou) {
  ...
  restartExecution({ executionId, principalId }) {
    if (!this.db || this.readOnly) return null
    return this.#transaction(db => {
      const execution = db.prepare('SELECT * FROM executions WHERE id = ?').get(executionId)
      if (!execution) throw new Error('no such execution')
      this.#requireMembership(execution.channel_id, principalId)
      if (!['running', 'error', 'killed'].includes(execution.state)) {
        throw new Error('only a running, failed, or stopped turn can be restarted')
      }
      const outbox = db.prepare('SELECT id FROM channel_outbox WHERE execution_id = ?').get(executionId)
      if (!outbox) throw new Error('this turn has already been retried')
      const restartedAt = nowIso()
      db.prepare(`
        UPDATE executions SET state = 'error', error = 'Restarted by a channel member', completed_at = ?
        WHERE id = ?
      `).run(restartedAt, executionId)
      db.prepare(`
        UPDATE channel_outbox
        SET state = 'pending', execution_id = NULL, available_at = ?, completed_at = NULL,
          last_error = 'Restarted by a channel member'
        WHERE id = ?
      `).run(restartedAt, outbox.id)
      db.prepare(`UPDATE threads SET state = 'working', updated_at = ? WHERE root_message_id = ?`)
        .run(restartedAt, execution.thread_root_id)
      return {
        sessionId: execution.session_id,
        channelId: execution.channel_id,
        threadRootId: execution.thread_root_id,
  ...
  recordPresentationFeedback({ channelId, principalId, planId, focus, action, idempotencyKey }) {
    this.#requireMembership(channelId, principalId)
    const cleanPlanId = boundedText(planId, 'presentation plan id', 128)
    const cleanFocus = String(focus || '')
    const cleanAction = String(action || '')
    if (!PRESENTATION_FOCUSES.has(cleanFocus)) throw new Error('invalid presentation focus')
    if (!PRESENTATION_FEEDBACK_ACTIONS.has(cleanAction)) throw new Error('invalid presentation feedback action')
    return this.#transaction(db => {
      const appended = this.#appendEvent(db, {
        channelId,
        type: 'presentation.feedback',
        actorId: principalId,
        idempotencyKey: String(idempotencyKey || randomUUID()).slice(0, 200),
        publicData: { planId: cleanPlanId, focus: cleanFocus, action: cleanAction },
      })
      return {
        eventId: appended.event.event_id,
        acceptedAt: appended.event.accepted_at,
        existing: appended.existing,
      }
    })
  }

  presentationFeedbackSummary(channelId, principalId) {
    this.#requireMembership(channelId, principalId)
    const rows = this.#requireDb().prepare(`
      SELECT public_json, accepted_at
      FROM events
      WHERE channel_id = ? AND type = 'presentation.feedback'
      ORDER BY seq DESC
      LIMIT 200
    `).all(channelId)
    const actions = {}
    const focuses = {}
    for (const row of rows) {
      const data = parseJson(row.public_json, {})
      if (PRESENTATION_FEEDBACK_ACTIONS.has(data.action)) actions[data.action] = (actions[data.action] || 0) + 1
      if (PRESENTATION_FOCUSES.has(data.focus)) focuses[data.focus] = (focuses[data.focus] || 0) + 1
    }
    return {
      sampleSize: rows.length,
      actions,
      focuses,
      latestAt: rows[0]?.accepted_at || null,
    }
  }
